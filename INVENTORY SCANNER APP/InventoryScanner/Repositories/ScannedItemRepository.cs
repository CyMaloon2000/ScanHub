...
namespace InventoryScanner.Repositories
{
    ...
    public class ScannedItemRepository : IScannedItemRepository
    {
        ...
        public async Task<Dictionary<int, int>> GetScanCountsBySessionAsync()
        {
            var db = await _databaseService.GetConnectionAsync();
            var rows = await db.QueryAsync<SessionScanCount>(
                "SELECT SessionId, COUNT(*) AS ScanCount FROM ScannedItem GROUP BY SessionId");

            return rows.ToDictionary(x => x.SessionId, x => x.ScanCount);
        }
